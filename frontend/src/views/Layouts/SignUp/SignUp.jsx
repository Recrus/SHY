...
function SignUp() {
    ...
    return (
        <div className='h-screen w-screen dark:bg-dark-blue ease-in-out duration-300'>
            <div
                className='max-w-[370px] mx-auto flex justify-center flex-col font-primary pt-20 md:max-w-[680px] 2xl:max-w-[800px]'>
                ...
                <div className='flex justify-around items-center mt-[20px]'>
                    <Checkbox defaultChecked inputRef={permission} className='bg-neutral checked:bg-accent dark:checked:bg-dark-purple'/>
                    <div className='text-[12px] md:text-sm 2xl:text-base dark:text-gray'>I want to receive inspiration,
                        marketing promotions and updates via email.
                    </div>
                </div>
                <button onClick={submitHandler}
                        className='rounded bg-accent text-neutral uppercase p-3 text-base mt-[20px] drop-shadow-md 2xl:text-lg dark:bg-dark-purple'>Sign
                    up
                </button>
                <Link to='/login'
                      className='text-gray no-underline self-end mt-[20px] text-[12px] cursor-pointer md:text-sm 2xl:text-base'>Already
                    have an
                    account? Sign in</Link>
                <div className='text-[10px] text-gray text-center mt-[34px] md:text-[12px]'>Copyright ©
                    SHY {new Date().getFullYear()}</div>
