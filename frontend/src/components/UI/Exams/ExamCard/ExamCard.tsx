...
import { ExamCardProps, LinkData } from "../../../../../types/types";
...
const ExamCard: React.FC<ExamCardProps> = ({ exam, setRefetch }) => {
    ...
    const copyToClipboard = async () => {
        try {
            ...
            await navigator.clipboard.writeText(res.data.data[0].link);
            setOpenAlert(true);
            setTimeout(() => {
                setOpenAlert(false);
            }, 1500);
    ...
    return (
        <div className="flex justify-center lg:justify-normal">
            <Card className="mt-6 lg:w-11/12 dark:bg-dark-accent">
                <CardBody className="flex flex-col items-center text-theme md:flex-row">
                    <div className="max-w-[90%]">
                        <div className="flex items-center mb-2">
                            <Typography
                                variant="h5"
                                className="font-primary mr-2"
                            >
                                {exam.name}
                            </Typography>
                            {user?.role_id === 1 ? (
                                <div className="flex">
                                    <Link
                                        ...
                                        state={{ exam: exam }}
                                    >
                                        <PencilIcon className="h-4 w-4 mr-2 hover:text-primary transition-all" />
                                    </Link>
                                    <button onClick={handleDeleteExam}>
                                        <TrashIcon className="h-4 w-4 hover:text-primary transition-all" />
                                    </button>
                                </div>
                            ) : null}
                            {link ? (
                    ...
                    <div className="flex-center mt-2">
                        <div
                            className={`w-14 h-14 rounded-full text-neutral flex-center items-center ${exam.colorClass}`}
                        >
                            {exam.avgMark
